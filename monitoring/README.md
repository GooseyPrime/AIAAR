...
`MAKE_WEBHOOK_BASE_URL` is only a host-level fallback probe. It treats any non-`000`, non-5xx HTTP response as proof that the host is reachable, not that a specific scenario path is healthy. Set `MAKE_HEALTHCHECK_URL` if you want the script to validate a specific production Make.com endpoint path.
