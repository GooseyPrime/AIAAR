...
- Airtable table-read success against a designated existing table using `AIRTABLE_API_KEY`, `AIRTABLE_BASE_ID`, `AIRTABLE_HEALTHCHECK_TABLE` (defaults to `Target Items`), and an optional `AIRTABLE_API_BASE_URL` prefix override before the base ID, which also surfaces auth/permission problems separately from missing-table problems
...
`MAKE_WEBHOOK_BASE_URL` is only a host-level fallback probe. It treats 2xx responses plus common redirect responses (`301`, `302`, `307`, `308`) as proof that the host is reachable, not that a specific scenario path is healthy. Set `MAKE_HEALTHCHECK_URL` if you want the script to validate a specific production Make.com endpoint path.
